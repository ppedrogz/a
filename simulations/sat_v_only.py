...
# ===================== Helpers robustos p/ órbita circular/equatorial ======================
_EPS_E = 1e-5       # limiar para tratar como (quase) circular
_EPS_I = np.deg2rad(1e-3)  # limiar para tratar como (quase) equatorial (em rad)
...
def _argument_of_latitude_deg(r_vec, v_vec):
    ...
    Se também for equatorial (i ~ 0), retorna a longitude verdadeira l_true.
    """
    ...
    # inclinação aproximada para decidir equatorial
    i_rad = np.arccos(np.clip(h[2]/hnorm, -1.0, 1.0))
    if i_rad <= _EPS_I or nnorm < 1e-14:
        # equatorial → longitude verdadeira l_true = atan2(y, x)
        l_true = np.degrees(np.arctan2(r[1], r[0])) % 360.0
        return l_true

    # caso geral (não equatorial): u = angle(n, r) no plano orbital
...
for k in range(X.shape[1]):
    ...
    # elementos completos (mantém sua API atual)
    el = get_orbital_elements(r_vec, v_vec, mu)
    orbital_elementss.append(el)

    # escolha robusta entre ν e u para séries angulares:
    e_now = get_eccentricity(r_vec, v_vec, mu)
    if e_now < _EPS_E:
        nu_or_u = _argument_of_latitude_deg(r_vec, v_vec)  # usa u (ou l_true se equatorial)
    else:
        nu_or_u = get_true_anomaly(r_vec, v_vec, mu)      # sua função padrão (0..360)
...
# Energia específica (se você tem get_specific_energy, use; senão, calcula)
try:
...
def simulate():
    ...
    onde:
      ...
      - nus_deg: ângulo de fase em graus (ν ou u), já em [0, 360)
