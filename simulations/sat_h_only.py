...
t = np.linspace(0, 4320, 100000)  # 50 dias, 1e6 pontos (como você mandou)
earth_radius = 6378.0  # km
mu = 3.986e5

# ===================== Dados de propulsão =====================
T   = 1.1e-3       # N (força total disponível)
Isp = 2150.0       # s
g0  = 9.80665      # m/s^2
m0  = 20.0         # kg
m_dry = 15.0       # kg
...
# ===================== Apenas motor H (perpendicular) =====================  # <-- alterado
# Removemos a lógica de dois motores. Só existe empuxo na direção w_hat.
T_H   = T                     # todo empuxo disponível é perpendicular         # <-- alterado
Isp_H = Isp                   # Isp do motor H                                 # <-- alterado

# ===================== Janelas em anomalia verdadeira (para H) =====================
THRUST_INTERVAL_DEG = 30.0
MEAN_THETA_LIST_DEG = [180]  # 180 apogeu - 0 perigeu (você escolhe)

def throttle(t, x):
    # Se não há empuxo disponível, desliga
    if T_H <= 0.0:                                               # <-- alterado (T_H)
        return 0.0
    # Queima só com propelente
...
def x_dot(t, x):
    ...
    # Empuxo exclusivamente perpendicular (H)
    u = throttle(t, x)
    aH = (T_H / m_cur) / 1000.0 if fire_H else 0.0                    # km/s^2  # <-- alterado
    SIGN_H = -1.0  # sentido "UP" (ajuste se quiser "DOWN")

    if u > 0.0 and aH > 0.0:                                                    # <-- alterado
        xdot[3:6] += SIGN_H * aH * w_hat                                        # <-- alterado (sem componente s_hat)
        mdot_H = T_H/(Isp_H*g0)
        xdot[6] = - mdot_H
    else:
        xdot[6] = 0.0
...
# Aceleração instantânea H (km/s^2)
aH_series = (T_H / np.maximum(m_series, 1e-18)) / 1000.0                 # <-- alterado

# Δv_H (km/s) → integração
dv_H_kms = float(np.sum(aH_series[:-1] * dt * (fire_mask[:-1] & u_mask[:-1])))  # <-- alterado

# Conversões p/ m/s
dv_H_ms = 1000.0 * dv_H_kms                                              # <-- alterado
dv_V_ms = 0.0                                                            # <-- alterado
dv_total_ms = dv_H_ms                                                    # <-- alterado
...
arg = np.clip((dv_H_kms) / (2.0 * v_ref), -1.0, 1.0)   # dv_H_kms em km/s
delta_i_ideal_deg = float(np.degrees(2.0*np.arcsin(arg)))
delta_i_sim_deg = incs_deg - incs_deg[0]

print("\n=== Dados H only (perpendicular) ===")                                # <-- alterado
print(f"Tempo com H ligado (s):     {t_H_on:.6f}")
print(f"Δv_H     (m/s):             {dv_H_ms:.6f}")                           # <-- alterado
print(f"Δv_total (m/s):             {dv_total_ms:.6f}")                       # <-- alterado
...
ax.plot3D(X[0, :], X[1, :], X[2, :], 'r', label="Satélite H only")          # <-- alterado
ax.set_box_aspect([1, 1, 1])
ax.set_title("Órbita simulada - Satélite H only")                           # <-- alterado
ax.legend()
ax.axis('equal')
...
def simulate():
    """
    Executa a integração com o x_dot deste módulo (H only) e devolve:
